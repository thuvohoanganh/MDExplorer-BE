...
const gatherMissing = (req, res, next) => {
    ...
    folders.forEach(async (data_type, i) => {
        ...
        if (i === folders.length - 1) {
            fs.writeFile(path.join(statistic, "missingness.json"), JSON.stringify(final_data, null, 2), (error) => {
                if (error) {
...
const gatherWithin = async (req, res, next) => {
    ...
    folders.forEach(async (data_type, i) => {
        ...
        if (i === folders.length - 1) {
            fs.writeFile(path.join(statistic, "within_distribution.json"), JSON.stringify(final_data, null, 2), (error) => {
                if (error) {
                    console.log('An error has occurred ', error);
                    return;
                }
